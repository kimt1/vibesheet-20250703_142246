const existing = this.storage.getItem(SelectorVault.STORAGE_KEY);
  ...
  /**
   * Decrypt a ciphertext produced by `encrypt()`.  Accepts legacy payloads that
  ...
  async decrypt(cipher: string): Promise<any> {
    ...
    try {
      ...
      throw new InvalidCiphertextError('Ciphertext is not valid base64');
    }
    ...
    if (packed.length < minLen) {
      throw new InvalidCiphertextError('Ciphertext too short or malformed');
    }
    ...
    if (isV1) {
      ...
      if (iterations === 0) {
        throw new InvalidCiphertextError('Invalid iteration count');
      }
      offset += 4;
    }
    ...
    try {
      ...
      throw new InvalidCiphertextError('Authentication failed');
    }
    ...
    try {
      ...
      throw new InvalidCiphertextError('Plaintext decoding failed');
    }
  }
